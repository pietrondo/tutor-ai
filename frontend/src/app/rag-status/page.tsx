...
export default function RAGStatusPage() {
  ...
  const fetchRAGStatus = async () => {
    try {
      setError(null)
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/rag/status`)
      if (response.ok) {
  ...
  const fetchDocumentStats = async () => {
    ...
    try {
      const url = searchQuery
        ? `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/rag/documents/${selectedCourse}?search_query=${encodeURIComponent(searchQuery)}`
        : `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/rag/documents/${selectedCourse}`
