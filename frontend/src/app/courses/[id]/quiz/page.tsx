...
export default function QuizPage() {
  ...
  const fetchQuizzes = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/courses/${courseId}/quizzes`)
      const data = await response.json()
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const url = editingQuiz
        ? `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/courses/${courseId}/quizzes/${editingQuiz.id}`
        : `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/courses/${courseId}/quizzes`
  ...
  const handleDelete = async (quizId: string) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/courses/${courseId}/quizzes/${quizId}`, {
        method: 'DELETE',
      })
