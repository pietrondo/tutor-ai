...
export default function AdvancedSearchPage() {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      try {
        const response = await fetch(`http://localhost:8000/search/suggestions?q=${encodeURIComponent(query)}&limit=8`)
        if (!response.ok) return
  ...
  useEffect(() => {
    ...
    const loadBooks = async () => {
      try {
        const response = await fetch(`http://localhost:8000/courses/${selectedCourse}/books`)
        if (!response.ok) return
