  ...
  const fetchProgress = async () => {
    try {
      const [progressResponse, insightsResponse] = await Promise.all([
        fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/study-progress/${courseId}`),
        fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/study-insights/${courseId}`)
      ])
