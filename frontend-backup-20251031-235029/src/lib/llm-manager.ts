...
class LLMManager {
  ...
  private initializeProviders() {
    ...
    // Set fallback chain
    this.fallbackChain = ['local', 'openrouter', 'openai', 'anthropic']
  }
  ...
  async checkProviderAvailability(providerId: string): Promise<boolean> {
    ...
    try {
      const response = await fetch(`${provider.baseUrl || this.getDefaultBaseUrl(provider.type)}/models`, {
      ...
      console.error(`Provider ${providerId} unavailable:`, error)
